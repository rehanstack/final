...
import { Box, Database, Cpu, Lock, Zap, Cloud, Server, Sparkles } from 'lucide-react'
import { loadAnalysis } from '../lib/analysisState'

export default function Architecture() {
  const analysis = loadAnalysis()
  const customTable = analysis.customData?.tables?.[0]
  const isCustomLoaded = Boolean(analysis.customData)
  const layers = [
    {
      name: 'Frontend Layer',
    ...
    {
      name: 'Python AI Layer',
      tech: 'FastAPI + LangChain',
      description: 'REST API for specialized agent workflows',
    ...
    {
      name: 'AI Model',
      tech: 'Qwen Models (Groq/Ollama)',
      description: 'Large language models for reasoning',
      color: 'from-yellow-600 to-orange-500',
      icon: Cloud
    },
    {
      name: 'Data Layer',
      tech: 'PostgreSQL, MySQL, SQLite, MongoDB',
  ...
  return (
    <div className="pt-24 pb-20 px-4">
      ...
      >
        ...
        {/* Active Uploaded CSV Flow */}
        {isCustomLoaded && (
          <motion.div variants={itemVariants} className="glass-dark p-6 rounded-2xl border border-primary/40 shadow-glow mb-12">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="text-xl font-bold text-white flex items-center gap-2">
                  Active Execution Pipeline: {customTable?.name || 'Custom CSV'}
                  <span className="badge badge-primary text-[10px]">Live Data</span>
                </h2>
                <p className="text-xs text-gray-400">CSV ingested ➔ PapaParse Header Tokenizer ➔ Groq LLM Inference ➔ Interactive Studio Pinning</p>
              </div>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 pt-3 border-t border-white/10 text-xs text-center">
              <div className="p-2 bg-white/5 rounded-lg border border-white/10">
                <span className="text-gray-400 block text-[10px] uppercase">1. File Ingestion</span>
                <span className="text-white font-bold">{customTable?.name || 'CSV File'}</span>
              </div>
              <div className="p-2 bg-white/5 rounded-lg border border-white/10">
                <span className="text-gray-400 block text-[10px] uppercase">2. Math & Schema</span>
                <span className="text-secondary font-bold">{analysis.customData?.totalRecords || 150} Records</span>
              </div>
              <div className="p-2 bg-white/5 rounded-lg border border-white/10">
                <span className="text-gray-400 block text-[10px] uppercase">3. LLM Reasoning</span>
                <span className="text-accent font-bold">Qwen Models</span>
              </div>
              <div className="p-2 bg-white/5 rounded-lg border border-white/10">
                <span className="text-gray-400 block text-[10px] uppercase">4. Interactive Studio</span>
                <span className="text-primary font-bold">{analysis.dynamicCharts?.length || 4} Charts Pinned</span>
              </div>
            </div>
          </motion.div>
        )}
        ...
        <motion.div variants={itemVariants} className="glass-dark p-8 rounded-xl mb-16 border border-white/10">
          <div className="space-y-4">
            {[
              ...
              { title: 'Python AI Layer', detail: 'FastAPI REST API' },
              { title: 'Master Agent', detail: 'LangGraph orchestration' },
              { title: 'ChromaDB', detail: 'Vector store for RAG' },
              { title: 'AI Model', detail: 'Qwen (Groq/Ollama)' },
              { title: 'Data Layer', detail: 'PostgreSQL, MySQL, SQLite, MongoDB' }
        ...
        <motion.div variants={itemVariants} className="glass-dark p-8 rounded-xl border border-secondary/20 mb-16">
          ...
          <div className="space-y-4">
            {[
              ...
              { step: 7, title: 'Reasoning', desc: 'Qwen model generates insights with context' },
          ...
          <div className="glass-dark p-8 rounded-xl border border-white/10">
            ...
            <ul className="space-y-2 text-gray-300">
              ...
              <li className="flex items-center gap-2">
                ...
                FastAPI + LangGraph for AI orchestration
              </li>
              <li className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-primary"></span>
              ...
              <li className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-secondary"></span>
                Qwen Models (via Groq/Ollama) for reasoning
              </li>
              <li className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-accent"></span>
                PostgreSQL, MySQL, SQLite, MongoDB for data storage
              </li>
            </ul>
          </div>
