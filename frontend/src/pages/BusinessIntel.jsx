...
import { evaluateChartReadiness } from '../lib/contentReviewer'
...
function buildAutoCharts(allTables, fallbackSampleRows, availableColumns) {
  ...
  if (allTables.length > 1) {
    ...
    businessTables.forEach(t => {
      ...
      const avgCols = numCols.filter(c => /age|score|rating|percent/i.test(c))
      ...
      catCols.forEach(xCol => {
        ...
        avgCols.forEach(yCol => {
          const data = aggregateData(rows, xCol, yCol, 'avg').slice(0, 8)
          if (data.length > 0) {
            charts.push({
              id: `sugg-avg-${t.name}-${xCol}-${yCol}`,
              title: `${t.name}: Avg ${yCol.replace(/_/g, ' ')} by ${xCol.replace(/_/g, ' ')}`,
              type: data.length <= 5 ? 'pie' : 'bar', xAxis: xCol, yAxis: yCol,
              isAutoGenerated: true, data
            })
          }
  ...
  catCols.forEach(xCol => {
    ...
    avgCols.forEach(yCol => {
      const data = aggregateData(sampleRows, xCol, yCol, 'avg').slice(0, 8)
      if (data.length > 0) {
        charts.push({
          id: `sugg-avg-${xCol}-${yCol}`,
          title: `Avg ${yCol.replace(/_/g, ' ')} by ${xCol.replace(/_/g, ' ')}`,
          type: data.length <= 5 ? 'pie' : 'bar', xAxis: xCol, yAxis: yCol,
          isAutoGenerated: true, data
        })
      }
...
export default function BusinessIntel() {
  ...
  const autoCharts = useMemo(() => {
    if (!isCustom) return [];
    const generated = buildAutoCharts(allTables, sampleRows, availableColumns);
    return generated.map(chart => {
      const review = evaluateChartReadiness(chart, availableColumns, sampleRows);
      return { ...chart, review };
    });
          ...
          <motion.div variants={itemVariants} className="mb-8 p-5 glass-dark border border-white/10 rounded-2xl">
            ...
            <div className="flex flex-wrap gap-2 max-h-[160px] overflow-y-auto pr-2 custom-scrollbar">
              {filteredSuggestions.map(sugg => {
                const isRejected = sugg.review && sugg.review.status === 'rejected';
                const statusTitle = sugg.review 
                  ? (isRejected ? '⚠ Not Ready\n' + sugg.review.issues.join('\n') : '✓ Ready') 
                  : '';
                
                return (
                  <button 
                    key={sugg.id} 
                    onClick={() => !isRejected && handlePinSuggestion(sugg)}
                    disabled={isRejected}
                    title={statusTitle}
                    className={`px-3 py-1.5 rounded-full border border-white/10 text-xs font-semibold transition-all flex items-center gap-1.5 shrink-0 ${
                      isRejected 
                        ? 'bg-red-500/10 border-red-500/30 text-red-300/80 cursor-not-allowed' 
                        : 'bg-dark-800 hover:border-primary hover:bg-primary/10 text-gray-300 hover:text-white'
                    }`}
                  >
                    {!isRejected ? <span className="text-green-500 font-bold">✓</span> : <span className="text-red-400 font-bold">⚠</span>}
                    {sugg.title}
                  </button>
                )
              })}
