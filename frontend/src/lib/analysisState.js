...
export function createAnalysisForDataset(datasetKey = 'E-Commerce Dataset', customDetails = null) {
  ...
  return {
    ...
    metrics: {
      ...
      quality: isCustom ? (targetCustom?.qualityScore || 98) : (baseData.qualityScore || 91),
      anomalies: isCustom ? (targetCustom?.anomaliesCount || 0) : (baseData.anomaliesCount || 0),
      embeddings: isCustom ? (targetCustom?.ragChunks?.length || 150) : (baseData.vectorCount || 100),
    ...
    insights: isCustom ? (targetCustom?.insights?.insights || []) : (baseData.insights || []),
