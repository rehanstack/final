...
export const AGENT_PIPELINE_STEPS = [
  ...
  { id: 5, name: 'Reasoning Agent', icon: 'Zap', status: 'Generating Gemini insights', summary: 'Analyzed database patterns.', tasks: ['Formulating RAG prompts', 'Evaluating risks'] },
...
export function createAnalysisForDataset(datasetKey = 'E-Commerce Dataset', customDetails = null) {
  const isCustom = datasetKey === 'Custom CSV'
  const baseData = isCustom ? (customDetails?.customData || {}) : (DATASETS[datasetKey] || DATASETS['E-Commerce Dataset'])
  ...
  return {
    ...
    completedAgents: [],
    metrics: {
      tables: customDetails?.tablesCount || baseData.tablesCount || 1,
      relationships: baseData.relationshipsCount || 0,
      quality: baseData.qualityScore || 85,
      anomalies: baseData.anomaliesCount || 0,
      embeddings: baseData.vectorCount || 100,
      totalRecords: baseData.totalRecords || 100,
      totalSize: customDetails?.size || baseData.totalSize || '1 MB'
    },
    columnTypesBreakdown: baseData.columnTypesBreakdown || [],
    tables: baseData.tables || [],
    relationships: baseData.relationships || [],
    insights: baseData.insights || [],
    ragChunks: baseData.ragChunks || [],
    agents: AGENT_PIPELINE_STEPS.map((agent) => ({
      name: agent.name,
      summary: `Processing ${customDetails?.name || baseData.name || 'Dataset'}...`,
      output: {}
    })),
    // Store original custom data for completeAnalysis to use
    customData: isCustom ? customDetails?.customData : null
  }
}
...
export function completeAnalysis(state) {
  const isCustom = state.datasetKey === 'Custom CSV'
  ...
  return {
    ...
    metrics: {
      ...state.metrics,
      tables: isCustom ? (baseData.tables?.length || 1) : baseData.tablesCount,
      relationships: baseData.relationshipsCount || 0,
      quality: baseData.qualityScore || 85,
      anomalies: baseData.anomaliesCount || 0,
      embeddings: baseData.vectorCount || 100,
    ...
    insights: baseData.insights || [],
    ragChunks: baseData.ragChunks || [],
    businessMetrics: baseData.businessMetrics || DATASETS['E-Commerce Dataset'].businessMetrics,
    agents: agentOutputs,
...
export function queryRAGKnowledgeBase(queryText, currentDatasetKey = 'E-Commerce Dataset') {
  ...
  return {
    query: queryText,
    answer: `Grounded Gemini RAG Analysis for query "${queryText}": Analyzed ${baseData.tablesCount} tables across ${baseData.name}. Retrieved top ${matchedChunks.length} semantic vector chunks from ChromaDB.`,
    confidence: 94,
